...
export function DatasetModeChip({ mode }: { mode: "SAMPLE" | "IMPORTED REAL" | "LIVE" }) {
  const tone: ChipTone = mode === "LIVE" ? "success" : mode === "IMPORTED REAL" ? "brand" : "neutral";
  return (
    <span
      className={cn(
...
export function Metric({
  ...
  evidence?: { label: string; to: NonNullable<LinkProps["to"]> };
}) {
  return (
    <div className="cl-panel flex flex-col gap-2 p-4 transition-shadow duration-200 hover:shadow-cluster-sm">
      ...
      {evidence ? <EvidenceLink label={evidence.label} to={evidence.to} /> : null}
    </div>
  );
}
...
export function EvidenceLink({ label, to }: { label: string; to: NonNullable<LinkProps["to"]> }) {
  return (
    <Link
      to={to}
...
export function EmptyState({
  ...
  return (
    <div className="cl-panel flex flex-col items-center gap-4 px-6 py-16 text-center">
      ...
      {action ? <EvidenceLink label={action.label} to={action.to} /> : null}
    </div>
  );
}
...
export function ErrorState({
  ...
  return (
    <div className="cl-panel border-[rgba(217,45,32,0.25)] p-6">
      ...
      {action ? (
        <div className="mt-4">
          <EvidenceLink label={action.label} to={action.to} />
        </div>
      ) : null}
    </div>
  );
