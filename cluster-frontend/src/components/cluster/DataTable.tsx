...
export function ResponsiveDataList<T>({
  ...
  return (
    <ul className="space-y-3 md:hidden">
      {rows.map((row) => (
        <li key={rowKey(row)} className="cl-panel p-4" onClick={onRowClick ? () => onRowClick(row) : undefined}>
      ...
      {options.map((o) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(o)}
            className={cn(
              "min-h-9 rounded-full border px-3.5 py-1.5 text-[0.8125rem] font-semibold whitespace-nowrap transition-colors duration-200",
              active
