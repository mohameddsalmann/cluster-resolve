...
import type {
  ...
  SupplierProductReliabilityEvaluation,
} from './types';

const DAY_MS = 86_400_000;

/**
 * Evaluates reliability for a specific supplier × product combination.
 * Uses the same policy windows and trigger logic as the per-supplier engine,
 * but filters observations to only those containing the given productId.
 */
export function evaluateSupplierProductReliability(
  datasetId: string,
  supplierId: string,
  productId: string,
  ...
  // Filter to this supplier + product combination
  const supplierObs = observations.filter(
    (value) =>
      value.datasetId === datasetId &&
      value.supplierId === supplierId &&
      value.productIds.includes(productId)
  );

  const recent = supplierObs.filter((value) => {
    const placed = timestamp(value.placedAt);
    return placed >= recentStart && placed <= asOfTime;
  });
  const baseline = supplierObs.filter((value) => {
    const placed = timestamp(value.placedAt);
  ...
  const triggers = enoughData ? detectProductTriggers(recentMetrics, baselineMetrics, recent.length) : [];
...
function detectProductTriggers(
